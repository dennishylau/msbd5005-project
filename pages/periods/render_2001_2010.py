...
def render_2001_2010(dfc_china_data, dfc_imf_dot):
    ...
    for fig in figs:
        fig.update_layout(
            ...
            margin=dict(
                ...
                pad=0,
                autoexpand=False))
        fig.add_vline(x=2001, line_dash="dot", annotation_text="Joined WTO", annotation_position='top left')
    ...
    st.header("2001 - 2010")
    desc_col, fig_col = st.columns([1, 1])
    with desc_col:
        st.write('''
    ...
    with fig_col:
        ...
        data = update_data(dfc_imf_dot, chosen_country, chosen_year, chosen_top_n=10, chosen_bottom_n=10)
